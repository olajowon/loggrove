...
logging.basicConfig(
    ...
    filemode='w',
    format='{"datetime":"%(asctime)s", "lineno":"%(lineno)d", '
...
class Monitor(threading.Thread):
    ...
    def run(self):
        try:
            ...
            for item in alert_items:
                min_time = self.curr_time - item['check_interval'] * 60 + 60
                interval_counts = monitor_item_counts[item['id']][-(item['check_interval']+5):]
                ...
                if alerting:
                    ALERTING_QUEUE.put((item['dingding_webhook'], content))
        except Exception as e:
    ...
    def check_alerting(self, item, counts, min_time):
        ...
        if counts == []:
            ...
            content = '# Loggrove 告警\n主机: %s\n文件: %s\n匹配: %s\n' \
                            '时间: %s:00 至 %s:59\n统计: %d 次\n\n 注意: 统计异常 ！！！\n\n' % \
                            (self.host, self.filepath, item['search_pattern'], min_strtime,
                             self.curr_strtime, 'None')
        elif eval(item['trigger_format'].format(sum(counts))):
            alerting = True
            content = '# Loggrove 告警\n主机: %s\n文件: %s\n匹配: %s\n' \
                            '时间: %s:00 至 %s:59\n统计: %d 次\n公式: %s\n\n' % \
                            (self.host, self.filepath, item['search_pattern'], min_strtime,
                             self.curr_strtime, sum(counts), item['trigger_format'])
    ...
    def make_alert_items(self):
        ...
        alert_items = []
        for item in self.monitor_items:
            if item['alert'] == 1 and (self.curr_time + 60) % (item['check_interval'] * 60) == 0:
...
def main():
    ...
    scheduler.add_job(monitor, 'cron', minute='*/1', max_instances=8)
    scheduler.start()
